...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <>
      ...
      <aside
        className={`fixed lg:static inset-y-0 left-0 w-64 lg:w-60 h-screen bg-[#0d0f15] border-r border-zinc-800/80 flex flex-col justify-between shrink-0 select-none z-50 transition-transform duration-300 ease-in-out ${
      ...
      >
        ...
        <div className="p-3.5 space-y-3">
          ...
          {/* Sentinel Autonomous QA Trigger */}
          {onOpenBacktester && (
            <button
              onClick={() => {
                if (onCloseMobile) onCloseMobile();
              ...
              className="w-full flex items-center justify-between px-2.5 py-1.5 rounded-lg bg-indigo-950/30 border border-indigo-500/30 text-indigo-300 hover:bg-indigo-900/40 hover:text-white transition-all text-xs group"
            >
              <span className="flex items-center gap-2">
                <ShieldCheck className="w-3.5 h-3.5 text-indigo-400 group-hover:scale-110 transition-transform" />
                <span className="font-medium">Site QA Sentinel</span>
              </span>
